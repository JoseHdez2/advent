...
class LightMatrix
  ...
  def do_order
    ...
    case(@to_do)
    when "toggle"; (x1..x2).each { |x| (y1..y2).each { |y| @matrix[x][y] += 2 }}
    when "turn off"; (x1..x2).each { |x| (y1..y2).each { |y| @matrix[x][y] -= 1; @matrix[x][y] = 0 if @matrix[x][y] < 0 }}
  ...
  def count_lights
    count = 0
    (0...@y).each { |y| (0...@x).each { |x| count += @matrix[x][y] } }
    count
  end
end
...
puts "The total intensity is #{lights.count_lights}."
